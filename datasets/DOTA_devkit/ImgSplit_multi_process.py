...
class splitbase():
    def __init__(self,
        ...
        if not os.path.isdir(self.outlabelpath) and os.path.exists(self.labelpath):
...
def rm_background(root_dir):
    print('start removing background files...')
    ...
    fsize = [os.path.getsize(os.path.join(label_dir,f)) for f in label_files]
